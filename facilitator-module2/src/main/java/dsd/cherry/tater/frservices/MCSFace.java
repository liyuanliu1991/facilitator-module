...
/*
 * created by fanny, supported by Andrew. 2016_6_3.
 */


/*
 * if it has error that says rate limited, you can call this function 26 seconds later. or change the subscription key in mcsfunc. 
 */
public class MCSFace extends FRServiceHandler {
	...
	/*
	 * train: userID can be null when first call this function. 
	 * case1: return serviceResponded = false, trainSuccessed = true, when i don't create person successfully.
	 * case2: return serviceResponded = true, trainSuccessed = false, the person has been created successfully, but the totalPhotosNum(already added into the person) has not reached TRAINING_PHOTO_MINIMUM or train group has error or overtime. 
	 * case3: return serviceResponded = true, trainSuccessed = true, when the person has enough photos and train group is successful. 
	 */
	...
	public FRServiceHandlerTrainResponse train(String userID, List<ImageData> images) {
		...
		if (userID == null) {
			...
			System.out.println("Mcs train create userID is: "+userID);
		}
	...
	/*
	 * verify:
	 * case1: return serviceResponded = false, confidence = 0.0, when the image has error such as no face or wrong format, or person training status is false, or idendify has some error. 
	 * case2: return servieResponded = true, confidence = "actual" when the image is good and identify successes!
	 */
	...
	public FRServiceHandlerVerifyResponse verify(String personID, ImageData image) {
		...
		try{
			...
			if(jsonObject == null){
				...
				image.setAcceptedByService(false);
			...
			System.err.println("MCS verify: has an exception");
			e.printStackTrace();
		}
...
//		 System.out.println(images.get(1).getImageBinary().toString());
		 ...
		 FRServiceHandlerTrainResponse trainResponse = fr.train(null, images);
		 userId = trainResponse.getFRPersonID();
		 ...
		 FRServiceHandlerVerifyResponse verifyResponse = fr.verify(userId, image);
		 System.out.println(verifyResponse.getConfidence());
		 
		 MCSFunc mcs = new MCSFunc();
		 try{
			 mcs.deletePersonGroup(userId);
		 } catch (Exception e){
			 e.printStackTrace();
		 }
	}
